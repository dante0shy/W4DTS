...
from W4DTS.utils.pesudo_label_generator import (
    PesudoLabelGenerator_v1_8_ot_rev_o as PesudoLabelGenerator,
)
...
pretrain = True
training_epochs = 60
training_epochs_ls = 5

mid_data_base = os.path.join(
    data_base,
    config_yaml["mid_data_base_format"].format(os.path.dirname(os.path.abspath(__file__)).split("/")[-1]),
)
...
for epoch in range(epoch_s, training_epochs):
    ...
    if not epoch % update_rate and train_first:
        ...
        with torch.no_grad():
            with tqdm.tqdm(
                total=len(data_update.files)*(sample_interval//2)// data_update.batch_size#// data_train.batch_size
            ) as pbar:
                time_s = time.time()
                        ...
                        for pidx in range(predictions.C[:, 0].max() + 1):
                            ...
                            with torch.no_grad():
                                ...
                                if idx:
                                    ...
                                    lgs[pidx] = sm_index.shape[0] #+ ini_mid_gt_sv[pidx].shape[0]
                                    ...
                                    sv_prob_updated[get_index] =torch.eye(20)[mid_gt_sv[get_index].flatten()].cuda()
                            ...
                            sv_prob_p = torch.nonzero(torch.softmax(sv_prob, dim=1).max(1)[0] > 0.95)
                            # trust_svm = torch.unique(torch.cat([trust_svm.flatten(), sv_prob_p.flatten()]))
                            ...
                            sm_mean_features[pidx] = mean_features[sm_index].detach_().cpu()
                            sm_ori_coords[pidx] = ori_coords[sm_index].detach_().cpu()
                            sm_mid_gt_sv[pidx] = (sv_prob_updated)[sm_index].max(1)[1].detach_().cpu()


                            if sur_sv_feature.shape[0] > 10000:
                                sample_idx = torch.randperm(sur_sv_feature.shape[0])[
                                    :10000
                                ].long()
                                tmp1= torch.zeros([sur_sv_feature.shape[0]],dtype=torch.bool)
