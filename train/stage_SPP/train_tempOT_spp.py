...
from W4DTS.utils.pesudo_label_generator import (
    PesudoLabelGenerator_v1_8_ot_rev as PesudoLabelGenerator,
)
...
if not os.path.exists(log_pos):
    os.makedirs(log_pos, exist_ok=True)
log_path = os.path.join(log_pos, "snap")  # os.path.dirname(__file__)
if not os.path.exists(log_path):
    os.mkdir(log_path)
exp_name = "unet_{}_OT_x".format(data_train.scale[0])
...
for epoch in range(epoch_s, training_epochs):
    ...
    if not epoch % update_rate and train_first:
        ...
        if epoch:
            sort_mIoU = list(
                sorted(list(logs.items())[-5:], key=lambda x: x[1]["mIoU"])
            )[-1]
            reload_epoch = sort_mIoU[1]["epoch"]
            print(
                "reload {}".format(
                    os.path.join(log_dir, "net-%09d" % reload_epoch + ".pth")
                )
            )
            unet.load_state_dict(
                torch.load(os.path.join(log_dir, "net-%09d" % reload_epoch + ".pth"))
            )
            optimizer.load_state_dict(
                torch.load(
                    os.path.join(log_dir, "net-%09d" % reload_epoch + ".pth").replace(
                        "net-", "optim-"
                    )
                )
            )
        ...
        with torch.no_grad():
            with tqdm.tqdm(
                total=len(data_update.files)
                * (sample_interval // 2)
                // data_update.batch_size  # // data_train.batch_size
            ) as pbar:
                time_s = time.time()

                for i, batch_loader in enumerate(data_update.get_data_loader()):
                    sur_sv_feature_list = [
                        None for tmp in range(data_update.batch_size)
                    ]
                    ...
                    for idx, batch in enumerate(batch_loader):
                        ...
                        for pidx in range(predictions.C[:, 0].max() + 1):
                            ...
                            sv_info[3] = torch.unique(
                                torch.cat(sv_info[3]).long()
                            ).long()
                            ...
                            sv_gt = (
                                ...
                                .index_add_(
                                    ...
                                    torch.eye(data.N_CLASSES)[gt_all.long()].cuda(),
                                )
                            ...
                            with torch.no_grad():
                                ...
                                if idx:
                                    ...
                                    (
                                        ...
                                        lgs[pidx],
                                        val=True,
                                        tmp_flag=True if epoch else False,
                                        rev=batch["re"][pidx],
                                        iter=10,
                                    )

                                    lgs[pidx] = sm_index.shape[
                                        0
                                    ]  # + ini_mid_gt_sv[pidx].shape[0]

                                else:
                                    lgs[pidx] = (
                                        mid_gt_sv != 0
                                    ).sum()  # len(sv_info[3])
                                    get_index = torch.nonzero(mid_gt_sv != 0).flatten()
                                    (
                                        trust_svm,
                                        sur_sv_feature,
                                        sur_sv_feature_coords,
                                    ...
                                    sv_prob_updated[get_index] = torch.eye(20)[
                                        mid_gt_sv[get_index].flatten()
                                    ].cuda()
                                    ini_mean_features[pidx] = (
                                        mean_features[mid_gt_sv != 0].detach_().cpu()
                                    )
                                    ini_ori_coords[pidx] = (
                                        ori_coords[mid_gt_sv != 0].detach_().cpu()
                                    )
                                    ini_mid_gt_sv[pidx] = (
                                        mid_gt_sv[mid_gt_sv != 0].detach_().cpu()
                                    )
                            ...
                            sv_prob_p = torch.nonzero(
                                torch.softmax(sv_prob, dim=1).max(1)[0] > 0.95
                            )

                            label_trust_sv = (sv_prob_updated)[trust_svm.flatten()].max(
                                1
                            )[1]
                            sv_gt_trust = sv_gt.max(dim=1)[1][trust_svm.flatten()]

                            sm_mean_features[pidx] = (
                                mean_features[sm_index].detach_().cpu()
                            )
                            sm_ori_coords[pidx] = ori_coords[sm_index].detach_().cpu()
                            sm_mid_gt_sv[pidx] = (
                                (sv_prob_updated)[sm_index].max(1)[1].detach_().cpu()
                            )

                            if sur_sv_feature.shape[0] > 6000:
                                sample_idx = torch.randperm(sur_sv_feature.shape[0])[
                                    :6000
                                ].long()
                                tmp1 = torch.zeros(
                                    [sur_sv_feature.shape[0]], dtype=torch.bool
                                )
                                ...
                                (
                                    ...
                                    torch.clone(sur_sv_feature[tmp1]).cpu(),
                                    torch.clone(sur_sv_feature_coords[tmp1]).cpu(),
                                    torch.clone(sur_sv_gt[tmp1]).cpu(),
                                )
                            else:
                                tmp1 = torch.ones(
                                    [sur_sv_feature.shape[0]], dtype=torch.bool
                                ...
                                if tmp1.sum() > 0:
                                    ...
                                    (
                                        ...
                                        torch.clone(sur_sv_feature).cpu(),
                                        torch.clone(sur_sv_feature_coords).cpu(),
                                        torch.clone(sur_sv_gt).cpu(),
                                    )

                            sur_sv_feature_list[pidx] = torch.cat(
                                [sm_mean_features[pidx], sur_sv_feature_list[pidx]],
                                dim=0,
                            )
                            sur_sv_feature_coords_list[pidx] = torch.cat(
                                [sm_ori_coords[pidx], sur_sv_feature_coords_list[pidx]],
                                dim=0,
                            )
                            sur_sv_gt_list[pidx] = torch.cat(
                                [sm_mid_gt_sv[pidx], sur_sv_gt_list[pidx]], dim=0
                            )
                        ...
                        pbar.set_postfix(
                            {"Tacc": "{0:1.5f}".format(counts_a / counts_t),}
                        )
                        pbar.update(1)
                        del batch
        ...
        with tqdm.tqdm(total=len(data_train) // data_train.batch_size) as pbar:
            ...
            for i, batch in enumerate(data_train.get_data_loader()):
                for idx, pos in enumerate(range(1, data_train.window_size + 1)):
                    ...
                    loss = criterion(
                        torch.cat(n_p, dim=0)[y.long().flatten() != 0],
                        y.long().flatten()[y.long().flatten() != 0],
                    )
                    loss.backward()
                    optimizer.step()
